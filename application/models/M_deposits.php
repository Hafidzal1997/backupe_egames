...
class M_deposits extends CI_Model {
    ...
    function fetch_data_deposits($like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL)
    {
        $where                  = "";
        ...
        if( ! empty($like_value))
        {
            ...
            $sql .= " AND (
                        ...
                        OR t1.jumlah_deposit LIKE '%".$this->db->escape_like_str($jumlah_deposit)."%'
    ...
    function get_all_sum_deposits_by_kode_member($kode_member, $clients_id)
    {
        $this->db->select_sum('t1.jumlah_deposit');
    ...
    function get_total_all_deposits_accepted()
    {
        $this->db->select_sum('t1.jumlah_deposit');
        $this->db->where('t1.flag_delete', 'N');
        $this->db->where('t1.flag_status', 'A');
    ...
    function get_total_all_deposits_accepted_summary_dashboard($clients_id = null, $filter_tanggal = null, $tanggal_awal = null, $tanggal_akhir = null)
    {
        $where = "";
        ...
        $sql    = "SELECT SUM(t1.jumlah_deposit) as total
                    FROM deposits as t1
    ...
    function get_total_all_deposits_waiting()
    {
    ...
    function get_total_all_deposits_waiting_today()
    {
        $current_date = date('Y-m-d');
    ...
    function get_deposits_by($data)
    {
        return $this->db->get_where('deposits', $data);
    }

    function get_deposits_by_id($id)
    {
        $sql = "SELECT 
                t1.*
                , t4.nama_lengkap
    ...
    function update_batch_deposits_by($column, $data)
    {
        return $this->db->update_batch('deposits', $data, $column); 
    }

    function update_deposits_by_id($id, $data)
    {
        return $this->db->update('deposits', $data, array('id' => $id));
    }

    function add_deposits($data)
    {
    ...
    {
        return $this->db->delete('deposits', array('id' => $id));
    }

    function fetch_data_deposits_hari_ini($like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL)
    {
    ...
    function export_data_deposits_hari_ini(){
        $tanggal_hari_ini   = date('Y-m-d');
        $where              = " AND (DATE(t1.created_at) BETWEEN '".$tanggal_hari_ini."' AND '".$tanggal_hari_ini."') ";
