...
class Transaksi_withdraws extends CI_Controller {
    ...
    function __construct()
    {
        ...
        $this->contents = 'backoffice/transaksi_withdraws/';
    }
    ...
    function withdraws_json()
    {
        ...
        $fetch          = $this->M_withdraws->fetch_data_withdraws($requestData['search']['value'], $requestData['order'][0]['column'], $requestData['order'][0]['dir'], $requestData['start'], $requestData['length']);
        ...
        { 
            ...
            if($row->flag_status == 'A'){
                ...
                if($row->flag_delete == 'N'){
                    $option .= '<a href="javascript:;" class="dropdown-item btn-accept text-success" data-url="'.base_url('backoffice/transaksi-withdraws/accept/' . encode($row->id)).'" title="Accept Data">
                                    <strong>
                                        <i class="fa fa-check mr-2"></i> Accept
                                    </strong>
                                </a>
                                <a href="javascript:;" class="dropdown-item btn-reject text-danger" data-url="'.base_url('backoffice/transaksi-withdraws/reject/' . encode($row->id)).'" title="Reject Data">
                                    <strong>
                ...
                if($row->flag_delete == 'N'){
                    $option .= '<a href="javascript:;" class="dropdown-item  text-success btn-accept" data-url="'.base_url('backoffice/transaksi-withdraws/accept/' . encode($row->id)).'" title="Restore to Accept Data">
            ...
            if($row->flag_delete == 'N'){
                ...
                if($this->roles_id == 1){
                    ...
                    $option .= '<a href="'.base_url('backoffice/transaksi-withdraws/edit/' . encode($row->id)).'" class="dropdown-item text-dark" title="Ubah Data">
                                    <strong>
                                        <i class="fa fa-edit"></i> Ubah Data
                                    </strong>
                                </a>
                                <a href="javascript:;" class="dropdown-item text-danger btn-delete" data-url="'.base_url('backoffice/transaksi-withdraws/delete/' . encode($row->id)).'" title="Hapus Data">
                                    <strong>
                ...
                if($this->roles_id == 1){
                    ...
                    $option .= '<a href="javascript:;" class="dropdown-item text-primary btn-restore" data-url="'.base_url('backoffice/transaksi-withdraws/restore/' . encode($row->id)).'" title="Restore Data">
                                    <strong>
    ...
    {        
        ...
        if($this->form_validation->run() == FALSE){
            ...
            redirect('backoffice/transaksi-withdraws');
        }
    }
    ...
    function export_excel(){
        $registers      = $this->M_withdraws->export_data_withdraws();
        $spreadsheet    = new Spreadsheet();

        $spreadsheet->getDefaultStyle()->getFont()->setName('Calibri');
        $spreadsheet->getDefaultStyle()->getFont()->setSize(11);

        $sheet = $spreadsheet->getActiveSheet(0)->setTitle("LAPORAN DATA WITHDRAW");
        ...
        */ 
        ...
        header('Content-Disposition: attachment;filename="Laporan Daftar Withdraw - '.date('dMY_His').'.xlsx"');
