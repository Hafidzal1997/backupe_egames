...
class Deposits extends CI_Controller {
    ...
    function __construct()
    {
        ...
        $this->contents = 'backoffice/deposits/';
    }
    ...
    function deposits_json()
    {
        ...
        $fetch          = $this->M_deposits->fetch_data_deposits_hari_ini($requestData['search']['value'], $requestData['order'][0]['column'], $requestData['order'][0]['dir'], $requestData['start'], $requestData['length']);
        ...
        { 
            ...
            if($row->flag_status == 'A'){
                ...
                if($row->flag_delete == 'N'){                    
                    $option .= '<a href="javascript:;" class="dropdown-item text-success btn-accept" data-url="'.base_url('backoffice/deposits/accept/' . encode($row->id)).'" title="Accept Data">
                                    <strong>
                                        <i class="fa fa-check mr-2"></i> Accept
                                    </strong>
                                </a>
                                <a href="javascript:;" class="dropdown-item text-danger btn-reject" data-url="'.base_url('backoffice/deposits/reject/' . encode($row->id)).'" title="Reject Data">
                                    <strong>
                ...
                $option .= '<a href="javascript:;" class="dropdown-item text-success btn-accept" data-url="'.base_url('backoffice/deposits/accept/' . encode($row->id)).'" title="Restore to Accept Data">
                                <strong>
                                    <i class="fa fa-check mr-2"></i> Accept
                                </strong>
                            </a>
            ...
            if($row->flag_delete == 'N'){    
                if($this->roles_id == 1){
                    $option .= '<a href="'.base_url('backoffice/deposits/edit/' . encode($row->id)).'" class="dropdown-item text-dark" title="Ubah Data">
                                    <strong>
                                        <i class="fa fa-edit"></i> Ubah Data
                                    </strong>
                                </a>
                                <a href="javascript:;" class="dropdown-item text-danger btn-delete" data-url="'.base_url('backoffice/deposits/delete/' . encode($row->id)).'" title="Hapus Data">
                                    <strong>
                ...
                if($this->roles_id == 1){                    
                    $option .= '<a href="javascript:;" class="dropdown-item text-primary btn-restore" data-url="'.base_url('backoffice/deposits/restore/' . encode($row->id)).'" title="Restore Data">
                                    <strong>
    ...
    function export_excel(){
        $registers      = $this->M_deposits->export_data_deposits_hari_ini();
        $spreadsheet    = new Spreadsheet();

        $spreadsheet->getDefaultStyle()->getFont()->setName('Calibri');
        $spreadsheet->getDefaultStyle()->getFont()->setSize(11);

        $sheet = $spreadsheet->getActiveSheet(0)->setTitle("LAPORAN DATA DEPOSIT HARI INI");
        ...
        */ 
        ...
        header('Content-Disposition: attachment;filename="Laporan Daftar Deposit Hari Ini - '.date('dMY_His').'.xlsx"');
