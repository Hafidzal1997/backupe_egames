...
class Transaksi_deposits extends CI_Controller {
    ...
    function __construct()
    {
        ...
        $this->load->model(array('M_deposits', 'M_banks','M_permainans', 'M_members', 'M_clients'));
        $this->load->library(array('form_validation'));

        $this->roles_id = $this->session->userdata('roles_id');
        $this->layouts  = 'layouts/v_backoffice';
        $this->contents = 'backoffice/transaksi_deposits/';
    }
    ...
    function deposits_json()
    {
        (!$this->input->is_ajax_request() ? show_404() : '');

        $requestData    = $_REQUEST;
        $fetch          = $this->M_deposits->fetch_data_deposits($requestData['search']['value'], $requestData['order'][0]['column'], $requestData['order'][0]['dir'], $requestData['start'], $requestData['length']);
        ...
        { 
            ...
            if($row->flag_status == 'A'){
                ...
                if($row->flag_delete == 'N'){                    
                    $option .= '<a href="javascript:;" class="dropdown-item text-success btn-accept" data-url="'.base_url('backoffice/transaksi-deposits/accept/' . encode($row->id)).'" title="Accept Data">
                                    <strong>
                                        <i class="fa fa-check mr-2"></i> Accept
                                    </strong>
                                </a>
                                <a href="javascript:;" class="dropdown-item text-danger btn-reject" data-url="'.base_url('backoffice/transaksi-deposits/reject/' . encode($row->id)).'" title="Reject Data">
                                    <strong>
                ...
                $option .= '<a href="javascript:;" class="dropdown-item text-success btn-accept" data-url="'.base_url('backoffice/transaksi-deposits/accept/' . encode($row->id)).'" title="Restore to Accept Data">
                                <strong>
                                    <i class="fa fa-check mr-2"></i> Accept
                                </strong>
                            </a>
            ...
            if($row->flag_delete == 'N'){    
                if($this->roles_id == 1){
                    $option .= '<a href="'.base_url('backoffice/transaksi-deposits/edit/' . encode($row->id)).'" class="dropdown-item text-dark" title="Ubah Data">
                                    <strong>
                                        <i class="fa fa-edit"></i> Ubah Data
                                    </strong>
                                </a>
                                <a href="javascript:;" class="dropdown-item text-danger btn-delete" data-url="'.base_url('backoffice/transaksi-deposits/delete/' . encode($row->id)).'" title="Hapus Data">
                                    <strong>
                ...
                if($this->roles_id == 1){                    
                    $option .= '<a href="javascript:;" class="dropdown-item text-primary btn-restore" data-url="'.base_url('backoffice/transaksi-deposits/restore/' . encode($row->id)).'" title="Restore Data">
                                    <strong>
            ...
            $nestedData[]   = number_format($row->jumlah_deposit);
    ...
    {        
        ...
        if($this->form_validation->run() == FALSE){
            ...
            redirect('backoffice/transaksi-deposits');
        }
    }
    ...
    function edit($id = null)
    {
        ...
        if($this->form_validation->run() == FALSE){
            ...
            $edit = $this->M_deposits->get_deposits_by_id($id);
            ...
            $this->M_deposits->update_deposits_by_id($id, $update);
    ...
    function delete_all()
    {
        ...
        if($update){
            $this->M_deposits->update_batch_deposits_by('id', $update);
        }
    ...
    function export_excel(){
        $registers      = $this->M_deposits->export_data_deposits();
        $spreadsheet    = new Spreadsheet();

        $spreadsheet->getDefaultStyle()->getFont()->setName('Calibri');
        $spreadsheet->getDefaultStyle()->getFont()->setSize(11);

        $sheet = $spreadsheet->getActiveSheet(0)->setTitle("LAPORAN DATA DEPOSIT");
        ...
        */ 
        ...
        $sheet->setCellValue('L1', 'Jumlah Deposit');
        $sheet->setCellValue('M1', 'Waktu Deposit');
        $sheet->setCellValue('N1', 'Status Deposit');
        $sheet->setCellValue('O1', 'Status Deposit Datetime');
        ...
        if($registers){
            foreach($registers as $row_registers){
                ...
                $jumlah_deposit         = $row_registers->jumlah_deposit;
                ...
                $sheet->setCellValueExplicit('L' . $i, $jumlah_deposit,\PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
        ...
        header('Content-Disposition: attachment;filename="Laporan Daftar Deposit - '.date('dMY_His').'.xlsx"');
