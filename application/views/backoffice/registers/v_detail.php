<form action="<?php echo current_url();?>" method="post" enctype="multipart/form-data">
    ...
    <div class="form-group row ">
        ...
        <div class="col-md-9 col-sm-9">
            <?php echo ($edit->no_hp ? $edit->no_hp : '-');?>
        </div>
    </div>                              
    <div class="form-group row ">
        <label class="control-label col-md-3 font-weight-bold">
        ...
        <div class="col-md-9 col-sm-9">
            <?php echo ($edit->email ? $edit->email : '-');?>
        </div>
    </div>
    <div class="form-group row ">
        <label class="control-label col-md-3 font-weight-bold">
...
<script>
    $("#MyModalDetailNotifikasiTitle").html('Detail Notifikasi Register');

    $(document).ready(function() {
        var url_edit    = '<?php echo base_url('backoffice/registers/edit/' . encode($edit->id));?>';
        var url_accept  = '<?php echo base_url('backoffice/registers/accept/' . encode($edit->id));?>';
        var url_reject  = '<?php echo base_url('backoffice/registers/reject/' . encode($edit->id));?>';
        ...
        function append_data(){
            var get_url_detail_append = '<?php echo base_url('backoffice/registers/detail-append/' . encode($edit->id));?>';
            $.get(get_url_detail_append, function(response) {
                $("#Register_<?php echo $edit->id;?>").html(response); 
            });
        }
