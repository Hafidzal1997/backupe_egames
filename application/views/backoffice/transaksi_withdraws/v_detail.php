<form action="<?php echo current_url();?>" method="post" enctype="multipart/form-data">
    ...
    <div class="form-group row ">
        ...
        <div class="col-md-9 col-sm-9">
            <?php echo number_format( $edit->jumlah_withdraw);?>
        </div>
    </div>
    <div class="form-group row ">
        <label class="control-label col-md-3 font-weight-bold">
...
<script>
    $("#MyModalDetailNotifikasiTitle").html('Detail Notifikasi Withdraw');

    $(document).ready(function() {
        var url_edit    = '<?php echo base_url('backoffice/transaksi-withdraws/edit/' . encode($edit->id));?>';
        var url_accept  = '<?php echo base_url('backoffice/transaksi-withdraws/accept/' . encode($edit->id));?>';
        var url_reject  = '<?php echo base_url('backoffice/transaksi-withdraws/reject/' . encode($edit->id));?>';
        ...
        function append_data(){
            var get_url_detail_append = '<?php echo base_url('backoffice/transaksi-withdraws/detail-append/' . encode($edit->id));?>';
            $.get(get_url_detail_append, function(response) {
                $("#Withdraw_<?php echo $edit->id;?>").html(response); 
            });
        }
