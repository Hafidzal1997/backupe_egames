...
<div class="py-2 text-white mb-3 font-weight-bold text-left" style="font-size: 12px;">
    Mohon Anda mengisi data dengan benar untuk memudahkan kami mengecek form deposit yang Anda kirim.
</div>
<form action="<?php echo current_url();?>" method="post">
    ...
    <div class="form-group text-left">
        <label class="font-weight-bold" for="jumlah_deposit">
            Jumlah Deposit <span style="color:red;">*</span> 
            <small><br>(Misalnya: 100000) </small>
        </label>
        <div>
            <input type="number" class="form-control" value="<?php echo set_value('jumlah_deposit');?>" name="jumlah_deposit" id="jumlah_deposit" autocomplete="off" required>
            <?php echo form_error('jumlah_deposit', '<strong class="text-danger">', '</strong>');?>
        </div>        
    </div>
    <div class="form-group text-left">
...
<script>
    $(".request-code").click(function(){
        ...
        $.ajax({
            url:"<?php echo base_url('sites/ajax-deposit-captcha-code');?>",
            type:"GET",
